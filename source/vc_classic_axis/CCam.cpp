...
int CCam::m_dwTimeToRestoreMove;
...
static CPatch InjectPatches([]() {
	...
	if (GetSetting.bProcess_FollowVeh) {
		...
		CPatch::RedirectCall(0x483B3B, CCam::ProcessFollowVeh);
	}
});
...
void CCam::ProcessFollowPedStandard(CVector const& target, float targetOrient) {	
	...
	if (!Pads->m_bDisablePlayerControls) {
		if (!TheCamera.m_bCamDirectlyBehind && !TheCamera.m_bCamDirectlyInFront && !LookingBehind && !LookingLeft && !LookingRight) {
			...
			if (pXboxPad->HasPadInHands()) {
				Alpha -= Pads->LookAroundUpDown() * 0.5f * fFV * (GetSetting.fRightStickVerticalSensitivity / 1280.0f);
				Beta -= Pads->LookAroundLeftRight() * 0.5f * fFV * (GetSetting.fRightStickHorizontalSensitivity / 1280.0f);
			}
		}
	}
	...
	if (CWorld::ProcessLineOfSight(TargetCoors, Source, colpoint2, e2, true, true, true, true, false, true, true)) {
		Source = colpoint2.m_vPoint;
...
void CCam::ProcessFollowVehicleStandard(CVector const& atarget, float targetOrient) {
	...
	if (FindPlayerVehicle() && FindPlayerVehicle()->m_nVehicleClass == VEHICLE_BIKE) {
		...
		if (TheCamera.CarZoomValueSmooth <= 1.0f)
			TheCamera.CarZoomValueSmooth = 1.0f;
	...
	if (TheCamera.CarZoomValueSmooth >= 6.0f)
		TheCamera.CarZoomValueSmooth = 6.0f;
